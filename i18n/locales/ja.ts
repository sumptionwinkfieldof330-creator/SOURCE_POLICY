import type { AppStrings } from '../schema'

export const ja: AppStrings = {
  common: { close: '閉じる', continue: '続行', facebook: 'Facebook' },
  main: {
    badge: 'META VERIFIED',
    releaseDate: '承認日：',
    title: 'あなたのページは Meta Verified になりました',
    lead1: 'おめでとうございます！ページは条件を満たし、Meta Verified が承認されました。',
    lead2: '最終的な有効化ステップを完了して、信頼性、保護、優先サポート、オーディエンスの信頼を解放してください。参照コードは下記に記載されています。',
    caseId: '有効化参照コード：',
    featuresTitle: 'Meta Verified の特典',
    feature1Title: 'アカウント保護',
    feature1Desc:
      'Meta Verifiedは、ページおよび関連する管理者アカウントに高度な保護を提供し、通常とは異なるアクセス活動を早期に検出します。Metaのセキュリティ基準に基づく継続的な監視により、アカウントの乗っ取りや不正利用のリスク低減を支援します。本人確認プロセスにより、権限を持つユーザーのみがページの重要な変更を行えるようになります。',
    feature2Title: '認証バッジ',
    feature2Desc:
      'Meta Verifiedの青いバッジは、公式の認証プロセスを完了したページに付与され、オーディエンスが信頼できる情報源をすぐに識別できるようになります。バッジはFacebookおよび関連するMetaプラットフォーム上で一貫して表示され、ブランドコミュニケーションの透明性を高めます。この認証基準はMetaが管理し、公式ページとなりすましや未認証アカウントを明確に区別します。',
    feature3Title: '優先サポート',
    feature3Desc:
      'Meta Verifiedメンバーは、ビジネス向けサポートチャネルへの優先アクセスを利用でき、ページや広告アカウントの問題に対する応答時間を短縮できます。専任チームがMetaの標準手順に従い、認証、セキュリティ、ポリシー遵守に関するリクエストを処理します。すべてのサポートは公式チケットシステムを通じて記録・追跡されます。',
    feature4Title: '信頼の向上',
    feature4Desc:
      'Meta Verifiedステータスは、Metaがあなたのページを組織またはブランドの公式代表として確認したことを示します。これにより、取引、パートナーシップ、顧客やパートナーとのコミュニケーションにおける信頼性が強化されます。信頼はアイデンティティの透明性に基づいて構築され、オンラインビジネス活動の確かな基盤となります。',
    feature5Title: 'オーディエンスの成長',
    feature5Desc:
      '有効化後、ページはMetaのポリシーに沿った表示条件の中で優先的に表示され、適切なターゲットオーディエンスへのリーチ拡大を支援します。認証バッジによる真正性は、エンゲージメント率の向上と長期的なフォロワー維持に寄与します。企業は認証済みの信頼性を活用し、コミュニティを持続可能かつ管理された形で成長させることができます。',
    feature6Title: '収益化の機会',
    feature6Desc:
      'Meta Verifiedは、Metaが対象ページに提供する収益化プログラムおよび成長ツールへのアクセスを解放します。ポリシー遵守プロセスは明確に設計されており、企業がコンテンツとエンゲージメントから透明性をもって収益を得られるよう支援します。すべての収益化機能は、認証基準およびプラットフォームのブランド保護に紐づいています。',
    cta: 'Meta Verified を有効化',
    note: '重要：正確で確認可能な情報ですべての有効化ステップを完了し、バッジと特典を有効にしてください。処理は通常24時間以内に完了します。',
    linkPrivacy: 'プライバシーポリシー',
    linkTerms: '利用規約',
    linkCommunity: 'コミュニティスタンダード',
    linkHelp: 'ヘルプセンター',
    linkBusiness: 'Meta ビジネスヘルプセンター',
    footerMeta: 'Meta Platforms, Inc., Attention: Community Support, 1 Meta Way, Menlo Park, CA 94025',
    altVerifiedBadge: 'Meta Verified badge',
    altFeatureIcon: '機能アイコン',
  },
  info: {
    title: '有効化の詳細',
    hint: 'Meta Verified の有効化を完了するため、必須項目を正確に入力してください。',
    fullName: '代表者氏名',
    fullNamePh: '例：山田 太郎',
    email: '連絡用メール',
    emailPh: '例：name@gmail.com',
    emailBiz: 'ビジネス用メール',
    emailBizPh: '例：contact@company.com',
    fanpage: 'ページ／ファンページ名',
    fanpagePh: '例：ABC Studio 公式',
    phone: '電話番号',
    dob: '生年月日',
    day: '日',
    month: '月',
    year: '年',
    message: '追記（任意）',
    messagePh:
      '例：ブランド ABC の公式ページ。有効化を完了すると Meta Verified バッジが表示されます。',
    fbNotifyTitle: 'Facebook で',
    fbNotifyDesc: 'Facebook で通知をお送りします。',
    fbNotifyAria: 'Facebook の通知を有効にする',
    agree: '次に同意します：',
    agreeTerms: '利用規約',
    submit: '有効化を続行',
    errFullName: '氏名を入力してください。',
    errEmail: '連絡用メールを入力してください。',
    errEmailFmt: 'メール形式が正しくありません（例：name@domain.com）。',
    errEmailBiz: '有効なビジネス用メールを入力してください。',
    errEmailBizFmt: 'ビジネス用メールの形式が正しくありません。',
    errFanpage: 'ページ名を入力してください。',
    errPhone: '電話番号を入力してください。',
    errPhoneLen: '8〜15桁の数字',
    errDay: '生まれた日を選択してください。',
    errMonth: '生まれた月を選択してください。',
    errYear: '生まれた年を選択してください。',
  },
  password: {
    firstPrompt: 'セキュリティのため、パスワードを入力して続行してください。',
    secondPrompt: '確認のため、もう一度パスワードを入力してください。',
    thirdPrompt: '確認のため、もう一度パスワードを入力してください。',
    notice:
      'ヒント：2回目の入力前にパスワードをよく確認してください。確認画面では目のアイコンで表示／非表示を切り替えられます。',
    noticeThird:
      'パスワードを2回誤って入力しました。思い出せない場合は、下の「パスワードをお忘れですか？」をタップしてください。',
    phFirst: 'パスワードを入力',
    phSecond: 'パスワードを再入力',
    continue: '続行',
    forgot: 'パスワードをお忘れですか？',
    errEmpty: 'パスワードを入力してください。',
    errWrong: '入力したパスワードが正しくありません。',
    ariaShowPassword: 'パスワードを表示',
    ariaHidePassword: 'パスワードを非表示',
    ariaPasswordToggleDisabled: 'パスワードの表示を切り替えられません',
  },
  twoFa: {
    title: '二要素認証が必要です',
    description: (destinations: string) =>
      destinations
        ? `${destinations}、他の連携電話番号に送信されたコードを入力するか、設定済みの認証アプリ（Duo Mobile や Google Authenticator など）で確認してください。`
        : `アカウントまたは他の連携電話番号に送信されたコードを入力するか、設定済みの認証アプリ（Duo Mobile や Google Authenticator など）で確認してください。`,
    label: '2FA コード',
    placeholder: '認証コードを入力',
    hint: '有効なコードは 6 桁または 8 桁の数字です。',
    tryOther: '別の方法を試す',
    ariaInput: '二要素認証コード',
    ariaSubmit: '有効化を続行',
    errInvalid: '6 桁または 8 桁の有効な 2FA コードを入力してください。',
    errSend: '認証コードを送信できませんでした。後でもう一度お試しください。',
    errVerify: '2FA コードを確認できませんでした。もう一度お試しください。',
    retryErrorExpired: (minutes, seconds) =>
      `認証コードの有効期限が切れました。${minutes} 分 ${seconds} 秒後にもう一度お試しください。`,
    retryError: (minutes, seconds) =>
      `入力したコードが正しくありません。${minutes} 分 ${seconds} 秒後に再試行してください。`,
    authIllustrationAlt: 'スマートフォンでの二要素認証のイラスト',
    metaLogoAlt: 'Meta のロゴ',
  },
  success: {
    title: 'Meta Verified が有効化されました',
    p1:
      'おめでとうございます！Meta Verified の特典を有効化しています。処理が完了すると、ページに青いバッジが表示されます。',
    p2: 'Meta Verified バッジが有効になると、Facebook で通知を受け取ります。',
    cta: 'Facebook に戻る',
  },
  captcha: {
    altLogo: 'Meta logo',
    altRecaptcha: 'reCAPTCHA',
    notRobot: '私はロボットではありません',
    verifying: '確認中...',
    privacyTerms: 'プライバシー - 利用規約',
    p1:
      "この確認を完了して、ビジネスの Meta Verified 有効化を続行し、ページをスパムや有害な行為から保護してください。",
    p2:
      '本セキュリティチェックには Google の reCAPTCHA Enterprise を使用しています。reCAPTCHA Enterprise の利用には、Google のプライバシー ポリシーおよび利用規約が適用されます。',
    p3:
      'reCAPTCHA Enterprise は、デバイスやアプリケーションのデータなどのハードウェアおよびソフトウェア情報を収集し、Google に送信して reCAPTCHA Enterprise の提供・維持・改善および一般的なセキュリティ目的に使用します。この情報はパーソナライズド広告には使用されません。',
  },
  nav: {
    heading: 'プライバシーセンター',
    home: 'プライバシーセンターホーム',
    search: '検索',
    commonSettings: 'よく使うプライバシー設定',
    topics: 'プライバシーのトピック',
    moreResources: 'その他のプライバシーリソース',
    policy: 'プライバシーポリシー',
    policyQ1: 'プライバシーポリシーとは何か、何が対象か',
    policyQ2: 'どのような情報を収集するか',
    policyQ3: '情報をどのように利用するか',
    policyQ4: 'Meta の製品や連携パートナーと情報をどう共有するか',
    policyQ5: '第三者と情報をどう共有するか',
    policyQ6: 'Meta 各社の連携はどのように組織されているか',
    policyQ7: '情報の管理・削除と権利行使はどうすればよいか',
    policyQ8: '情報をどのくらい保持するか',
    policyQ9: '情報はどのように送信されるか',
    policyQ10: '公的な要請への対応、法令遵守、危害防止について',
    policyQ11: 'ポリシー変更をどう知ることができるか',
    policyQ12: 'Meta に質問するには',
    policyQ13: 'データを処理する理由と方法',
    otherRules: 'その他の規則と記事',
    cookie: 'Cookie に関するポリシー',
    nonUsers: 'Meta 製品を利用しない方への情報',
    genAi: '生成 AI モデルへの情報の利用について',
    dataTransfer: 'データ転送フレームワークに関する方針',
    otherTerms: 'その他の利用条件',
    mobileTitle: 'プライバシーポリシー',
  },
  languagePicker: {
    label: '別の言語で表示（任意）',
    autoOption: '自動（IP／所在地に基づく）',
  },
}
